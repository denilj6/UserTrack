...
export default class LoadingPage extends Component<Props, State> {
    ...
    componentDidMount() {
        ...
        setTimeout(function timer() {
            AsyncStorage.getItem('isPermissionGiven').then(isPermissionGiven => {
                if (isPermissionGiven === '1') {
        ...
        return (
            <View style={{ flex: 1 }}>
                <Image style={{ flex: 1, width: Dimensions.get('window').width, height: Dimensions.get('window').height }}
                    ...
                    <Text style={{fontSize:50,fontWeight:'bold',textAlign:'center'}}> USER TRACK...</Text>
            </View>
        )
    }
}